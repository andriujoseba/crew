...
- the engine's own directories count as shipped content too: `~/duty/bin`, `lib`, `prompts` or `conf` redirected through a symlink now reads `modified`, and installing replaces the redirect with a real directory holding what was behind it, parking the link in `~/duty/legacy/` as evidence of where it pointed (#159)

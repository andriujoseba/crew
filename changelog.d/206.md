...
- Note: both `#206` entries above describe work that shipped in `0.1.0`. Their
  fragment went unconsumed because `0.1.0`'s release PR merged twelve commits
  stale (#212), so this is the first section that could carry them; the
  published `0.1.0` section is left as it stands (#162).

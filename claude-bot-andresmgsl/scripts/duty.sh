...
while IFS= read -r R; do
  ...
  if [ "$cr_count" = "err" ]; then
    ...
    dir="$WORK_DIR/$slug"
    ensure_main_clone "$R" "$dir" "$fork_url"
    run_session "$dir" build "You are the builder $ME in $R. Read AGENTS.md at the repo root, then act per BUILDER.md: first answer any round your open PRs owe — the whole round, one reply, then re-request every panel reviewer not approving the head you just pushed (your push staled the same-round approvals too, not just the changes-requests); otherwise pick ONE ready unclaimed issue (the roster in CONTRIBUTING points you at the release-flow and guards machinery), claim it, build it. Checkpoint discipline — the board and the branch are your only memory; assume this box can die at any moment, and anything uncommitted or not in the worklog is lost: open your PR as DRAFT as soon as the branch has its first commit, with a '## Worklog' section in the body listing your planned steps as checkboxes; check each item off and push every time you complete one. When the issue leaves you blocked or unsure, post the question as a comment on the issue and @-mention dan-claude-bot (triage owns the spec) — then keep working on what's unblocked, or wait, per BUILDER.md. Never guess your way past a spec gap. $WT_RULES $ROUND_RULES"
  else
    log "$R: no build duty"
  fi

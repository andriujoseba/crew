...
rehearsal_review_announce_precedes_verdict_from_json() {
  local identity="$1" head_sha="$2" comments_json="$3" reviews_json="$4"
  jq -en --arg identity "$identity" --arg head_sha "$head_sha" \
    --argjson comments "$comments_json" --argjson reviews "$reviews_json" '
      [$comments[]
        | select(.user.login == $identity and .body == ("🔎 reviewing head " + $head_sha))
...
rehearsal_review_announce_precedes_verdict() {
  local repo="$1" pr="$2" identity="$3" head_sha="$4" comments_json reviews_json
  ...
  rehearsal_review_announce_precedes_verdict_from_json \
    "$identity" "$head_sha" "$comments_json" "$reviews_json"
}

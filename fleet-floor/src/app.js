...
/* Shared, because all three stand on the same floor under the same lamp: the
   dark line where the body meets the deck, and the sliver of key light that
   lands along the top edge. */
function deckBase(x,w,ty,fy,lit){
  contactShadow(x+w/2,fy,w*0.86);
  S.fillStyle="rgba(0,0,0,0.45)";S.fillRect(x+3,fy-3,w-6,3);
  return function topSheen(col){S.fillStyle=rgba(col[0],col[1],col[2],0.10+0.16*lit);S.fillRect(x-3,ty,w+6,1.5);};
}
...
function fabTable(t,lit,st){
  ...
  var sheen=deckBase(x,w,ty,fy,lit);
  // drawer chest, left
  plate(S,[[x+2,ty+12],[x+104,ty+12],[x+104,fy],[x+2,fy]],"#2b2119","#150f0a","#3d2d1d");
  for(var d=0;d<3;d++){var dy=ty+18+d*12;
    S.fillStyle="#170f0a";S.fillRect(x+8,dy,90,10);
    S.fillStyle="rgba(190,140,80,0.55)";S.fillRect(x+40,dy+4,26,2);}
  S.fillStyle="rgba(255,205,150,"+(0.05+0.07*lit)+")";S.fillRect(x+2,ty+12,102,1.5);
  // open frame, right: two uprights and a diagonal brace
  S.fillStyle="#141b26";S.fillRect(x+w-56,ty+12,10,fy-ty-12);S.fillRect(x+w-14,ty+12,10,fy-ty-12);
  S.strokeStyle="#1f2937";S.lineWidth=4;S.beginPath();S.moveTo(x+w-48,fy-8);S.lineTo(x+w-8,ty+24);S.stroke();
  ...
  /* Top slab. The first pass made it the brightest thing in the room, which is
     the plank's mistake with better geometry — a horizontal surface under a
     lamp catches light, but it is not the subject. Dark body, and the light
     lands on the leading edge only. */
  plate(S,[[x-5,ty],[x+w+5,ty],[x+w+5,ty+9],[x-5,ty+9]],"#2c343e","#1a212a","#3d4854");
  plate(S,[[x-5,ty+9],[x+w+5,ty+9],[x+w+2,ty+14],[x-2,ty+14]],"#1a212a","#0f141b","#26303b");
  sheen([255,214,170]);
  ...
  plate(S,[[vx+9,vt-30],[vx+22,vt-30],[vx+22,vt-7],[vx+9,vt-7]],"#4d5866","#2c343f","#6a7787");   // stock
  ...
  // stock leaning against the chest, where offcuts actually end up
  S.strokeStyle="#39434f";S.lineWidth=4;
  S.beginPath();S.moveTo(x+118,fy-2);S.lineTo(x+128,ty+2);S.stroke();
  S.strokeStyle="#2e3742";S.lineWidth=3;
  S.beginPath();S.moveTo(x+126,fy-2);S.lineTo(x+134,ty+6);S.stroke();
  ...
  emit(function(c){var col=st==="working"?[247,189,78]:st==="offline"?[120,60,60]:[90,150,210];
    c.fillStyle=rgba(col[0],col[1],col[2],on?0.42:0.26);c.fillRect(mx+4,my+4,34,20);
    c.fillStyle=rgba(col[0],col[1],col[2],on?0.62:0.3);
...
function inspectBench(t,lit,st){
  ...
  plate(S,[[x+w-96,ty+12],[x+w-2,ty+12],[x+w-2,fy],[x+w-96,fy]],"#26303e","#121a24","#33404f");
  for(var d=0;d<3;d++){var dy=ty+18+d*12;
    S.fillStyle="#111823";S.fillRect(x+w-90,dy,82,10);
    S.fillStyle="rgba(170,196,224,0.45)";S.fillRect(x+w-58,dy+4,20,2);}
  S.fillStyle="#141d28";S.fillRect(x+4,ty+12,10,fy-ty-12);S.fillRect(x+w-104,ty+12,10,fy-ty-12);
  S.fillStyle="#1a2430";S.fillRect(x+4,ty+34,w-100,7);
  for(var p=0;p<4;p++){S.fillStyle=p%2?"#8b96a6":"#7a8695";S.fillRect(x+20+p,ty+30-p*3,44,3);}
  // glass top: a pale slab with a bright leading edge
  plate(S,[[x-5,ty],[x+w+5,ty],[x+w+5,ty+8],[x-5,ty+8]],"#43505f","#27313d","#5c6c7e");
  S.fillStyle="rgba(190,225,255,0.10)";S.fillRect(x-5,ty+1,w+10,3);
  plate(S,[[x-5,ty+8],[x+w+5,ty+8],[x+w+2,ty+13],[x-2,ty+13]],"#222b36","#141a22","#2e3846");
  sheen([196,232,255]);
  // lens arm and the specimen under it
  var ax=x+w-58,yl=ty-2;
  S.strokeStyle="#3a465e";S.lineWidth=2;S.beginPath();S.moveTo(ax,ty);S.lineTo(ax+4,yl-26);S.lineTo(ax+26,yl-35);S.stroke();
  S.fillStyle="#1a2230";S.beginPath();S.arc(ax+31,yl-35,9,0,7);S.fill();
  ...
  S.strokeStyle="rgba(196,232,255,0.5)";S.lineWidth=1;S.beginPath();S.arc(ax+31,yl-35,7,0,7);S.stroke();
  ...
  S.fillStyle=off?"#4a2a2a":"#5fce9b";S.fillRect(x+31,ty-9,16,4);
...
function plotTable(t,lit,st){
  ...
  // the tilted chart surface
  var fb=ty+14, bk=ty-10;
  plate(S,[[x-4,fb],[x+w+4,fb],[x+w-14,bk],[x+14,bk]],"#141d29","#080e16","#2c3b50");
  ...
  if(!off)emit(function(c){c.save();c.globalCompositeOperation="lighter";
    ...
    for(var b=0;b<5;b++){var ph=(t*0.35+b*0.2)%1, bx=x+18+ph*(w-36), by=bk+8+(fb-14-bk)*((b%3)/2.6);
      c.fillStyle="rgba(150,200,255,"+(0.35+0.45*Math.sin(t*3+b))+")";c.fillRect(bx,by,2.5,2.5);}
    c.restore();});
  ...
  for(var s2=0;s2<3;s2++){var sx=x+38+s2*58;
    S.fillStyle="#0e151e";S.fillRect(sx,fb+3,12,4);

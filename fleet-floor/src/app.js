...
function buildClaude(t,st){
  ...
  /* ---------- BACKPACK heat-sink (behind, drawn first) ----------
     It was drawn in the same tones as the front armour, so the part that
     survives behind the chest — a flat wing beside each arm — read as a grey
     rectangle floating at the torso's own depth. It is BEHIND: it gets the
     shade of a back plane, a sharper taper so the wings are shaped, fin cuts
     so they read as heat-sink, and the torso's own shadow falling on their
     inner edge, which is what actually pushes them a plane back. */
  var bpT=offl?"#0e131a":"#161d28", bpB="#060a10", bpE="#212a37";
  plate(g,[[cx-66,264+breath],[cx-38,248+breath],[cx+38,248+breath],[cx+66,264+breath],[cx+54,334+breath],[cx-54,334+breath]],bpT,bpB,bpE);
  [-1,1].forEach(function(sw){for(var fb=0;fb<3;fb++){var fy2=280+fb*16+breath;
    pl(g,cx+sw*46,fy2,cx+sw*(62-fb*2),fy2-6,rc,1.3);}});
  [-1,1].forEach(function(sw2){var sx0=cx+sw2*40;
    var shg=g.createLinearGradient(sx0,0,cx+sw2*66,0);
    shg.addColorStop(0,"rgba(0,0,0,0.45)");shg.addColorStop(1,"rgba(0,0,0,0)");
    g.fillStyle=shg;poly(g,[[sx0,250+breath],[cx+sw2*66,264+breath],[cx+sw2*56,334+breath],[sx0,334+breath]]);g.fill();});
  /* Exhaust stacks. They were straight rectangles with a lit square floating
     at the tip — behind the chest, only that tip survives, so each read as a
     glowing panel hanging beside the head with no pipe under it. Now each is
     a canted pipe with a slanted-cut hollow mouth, heat fins where the metal
     sheds, and a clamp band with a brace bolting it to the backpack — the tip
     explains the machine it is welded to. */
  [[-52,-1],[52,1]].forEach(function(k){var bx=cx+k[0],s=k[1],ty=breath;
    var tc=bx+s*6; // mouth centre, canted outboard
    plate(g,[[bx-s*11,250+ty],[tc-s*8,200+ty],[tc+s*8,192+ty],[bx+s*11,250+ty]],sT,sB,ed);
    // hollow mouth, cut on the same slant
    g.fillStyle="#03060c";poly(g,[[tc-s*8,200+ty],[tc+s*8,192+ty],[tc+s*8,197+ty],[tc-s*8,205+ty]]);g.fill();
    // ember in the throat, not a sticker on the face
    if(!offl)[RB,RE].forEach(function(c){var eg=c.createLinearGradient(0,193+ty,0,204+ty);
      eg.addColorStop(0,rgba(255,170,80,0.85));eg.addColorStop(1,rgba(200,80,20,0.15));
      c.fillStyle=eg;poly(c,[[tc-s*6,201+ty],[tc+s*6,194+ty],[tc+s*6,197+ty],[tc-s*6,204+ty]]);c.fill();});
    // heat fins: three cuts, each with a lit upper lip
    for(var f=0;f<3;f++){var fy=212+f*9+ty,fw=9+f*0.6,fcx=bx+s*(6-f*2);
      pl(g,fcx-fw,fy,fcx+fw,fy,rc,1.6);
      pl(g,fcx-fw,fy-1.2,fcx+fw,fy-1.2,"rgba(150,172,204,"+(offl?0.10:0.26)+")",1);}
    // clamp band + brace down to the backpack shoulder
    plate(g,[[bx-13,240+ty],[bx+13,240+ty],[bx+13,246+ty],[bx-13,246+ty]],sT,sM,ed);
    rivet(g,bx-8,243+ty,eH);rivet(g,bx+8,243+ty,eH);
    pl(g,bx+s*13,243+ty,bx+s*17,262+ty,"#0a0f18",3);
    pl(g,bx+s*13,243+ty,bx+s*17,262+ty,offl?"#2c3440":"#5b6c85",1);});
  ...
  function arm(sgn,mode){
    ...
    // piston from the socket down into the upper arm
    pl(g,shx,shy+14,shx+sgn*3,shy+32,"#0a0f18",5);
    pl(g,shx,shy+14,shx+sgn*3,shy+32,offl?"#39424f":"#8fa6c4",1.6);
    ...
    if(mode==='reachdown'){ // reach forward + down (welding pose)
      ...
      actuator(shx+sgn*12,shy+28,ex+sgn*5,ey-7);
      ...
      plate(g,[[fx-10,fy-4],[fx-17,fy+2],[fx-10,fy+8]],sM,sB,ed); // bracer fin, back edge
      ...
      actuator(shx+sgn*12,shy+28,ex3+sgn*5,ey3-7);
      ...
      plate(g,[[fx3+12,fy3],[fx3+19,fy3+6],[fx3+12,fy3+12]],sM,sB,ed); // bracer fin, back edge
      ...
      actuator(shx+sgn*12,shy+30,ex2+sgn*6,ey2-9);
      /* The forearm was a straight slab. A bracer: the outer edge flares out
         from the elbow and cuts back hard to the wrist, and a blade fin
         stands off the flare — the arm's silhouette finally has an opinion. */
      function ox(d){return ex2+sgn*d;}
      plate(g,[[ox(-14),ey2-2],[ox(14),ey2-2],[ox(19),ey2+26],[ox(13),ey2+58],[ox(-13),ey2+58],[ox(-15),ey2+30]],sT,sM,ed);
      plate(g,[[ox(17),ey2+6],[ox(26),ey2+18],[ox(17),ey2+32]],sM,sB,ed);
      pl(g,ox(14),ey2-2,ox(19),ey2+26,"rgba(150,172,204,"+(offl?0.10:0.26)+")",1.1);
      pl(g,ox(-6),ey2+8,ox(-6),ey2+50,rc,1);
      // power slot cut into the bracer face — the arm's one warm light
      g.fillStyle="#03060c";g.fillRect(ox(4)-2,ey2+14,4.5,24);
      [RB,RE].forEach(function(c){accGlow(c,ox(4)-1.2,ey2+16,2.8,20);});

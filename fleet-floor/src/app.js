...
function buildGrok(t,st){
  ...
  REARC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+8, rank=ci%3;
    ...
    var mx=hcx+s*(cb[1]+(rank===2?32:24)), my=offl?hby+62:hby+40;
  ...
  function leg(sgn){
    ...
    var kx=cx+sgn*(offl?52:(lead?54:46)), ky=(lead?452:456)+d*0.3;
    var ax=cx+sgn*(lead?40:24), ay=(lead?516:518)+d*0.1;
    var fx=cx+sgn*(lead?46:30), fy=546;
  ...
  function arm(sgn,mode){
    ...
    if(mode==='reachdown'){ /* builder: the FLEX — elbow thrown out past the
      ...
      g.save();g.globalCompositeOperation="source-atop";g.fillStyle="rgba(2,4,9,0.55)";
      poly(g,[[wx-11,wy+1],[wx+14,wy+1],[wx+17,wy+16],[wx+10,wy+34],[wx-4,wy+34],[wx-13,wy+16]]);g.fill();
      g.restore();
  ...
  // skull
  plate(g,[[hcx-22,hby+24],[hcx-19,hby+8],[hcx-9,hby],[hcx+9,hby],[hcx+19,hby+8],[hcx+22,hby+24],[hcx+17,hby+46],[hcx+10,hby+56],[hcx-10,hby+56],[hcx-17,hby+46]],hT,hM,ed);
  ...
  plate(g,[[hcx-18,hby+26],[hcx,hby+30],[hcx+18,hby+26],[hcx+14,hby+15],[hcx-14,hby+15]],hBr,hBrB,ed);
  ...
  // bezels: the optics are instruments seated in the recess — but the seat is
  // a shadow-line, not a goggle rim; only the lower lip catches anything
  [-1,1].forEach(function(s){g.fillStyle="#02040a";g.fillRect(hcx+s*8.5-4.5,hby+30,9,6);
    pl(g,hcx+s*8.5-4.5,hby+36,hcx+s*8.5+4.5,hby+36,"rgba(120,142,175,"+(offl?0.1:0.22)+")",0.9);});
  ...
  /* twin optics, deep-set. Idle they SCAN — brightness alternates side to
     side, slow, the way a thing watches a room it already owns. Working they
     both burn. Dead: two grey ticks, one holding a last ember. */
  [-1,1].forEach(function(s){var ox=hcx+s*8.5, oy=hby+33;
    [RB,RE].forEach(function(c2){ if(offl){c2.fillStyle="#20262e";c2.fillRect(ox-3.5,oy-1.5,7,3);
      ...
      // idle scan: both optics breathe TOGETHER — any per-side differential,
      // however small, makes a still frame read as two different eyes
      // instead of twin optics (the L2 verdict). Working keeps the phase
      // stagger: motion sells it there, a still never has to.
      var scn=0.62+0.2*Math.sin(t*2.1);
      var br2=work?(0.8+0.2*Math.sin(t*9+s)):scn;
      // hard point of light: bright core, tight falloff — a wide soft halo
      // here fogs the whole mask once the stage blooms it
      var eg=c2.createRadialGradient(ox,oy,0.5,ox,oy,6);
      eg.addColorStop(0,puh(0.98*br2));eg.addColorStop(0.35,pu(0.72*br2));eg.addColorStop(1,pu(0));
      c2.fillStyle=eg;c2.beginPath();c2.arc(ox,oy,6,0,7);c2.fill();
      c2.fillStyle=puh(0.95*br2);c2.fillRect(ox-2.6,oy-1,5.2,2.2);});});
  // cheek ridges: flaring OUT toward the jaw — the mandible echo, built in
  [-1,1].forEach(function(s){
    plate(g,[[hcx+s*19,hby+28],[hcx+s*12,hby+42],[hcx+s*15,hby+58],[hcx+s*23,hby+48]],hT,sB,ed);
    plate(g,[[hcx+s*23,hby+48],[hcx+s*28,hby+56],[hcx+s*16,hby+58]],hT,sB,null);
  ...
  // was re-seated after
  plate(g,[[hcx-13,hby+44],[hcx+13,hby+44],[hcx+15,hby+58],[hcx+9,hby+66],[hcx-9,hby+66],[hcx-15,hby+58]],hT,hM,ed);
  // the lower lip of the chin catches the lamp — the "rough chin" needs an edge
  pl(g,hcx-8,hby+64.5,hcx+8,hby+64.5,"rgba(170,192,222,"+(offl?0.1:0.28)+")",1.2);
  RB.fillStyle=rc;for(var vs=0;vs<3;vs++)RB.fillRect(hcx-6.5+vs*5,hby+50.5,3.4,7.5);
  ...
  for(var st2=0;st2<3;st2++)poly(g,[[hcx-6+st2*5,hby+58],[hcx-3.6+st2*5,hby+58],[hcx-4+st2*5,hby+64],[hcx-5.6+st2*5,hby+64]]),g.fill();
  g.restore();
  g.save();g.globalCompositeOperation="destination-out";
  poly(g,[[hcx-15,hby+56],[hcx-9,hby+58],[hcx-12,hby+63]]);g.fill();g.restore();
  pl(g,hcx-15,hby+56,hcx-12,hby+63,"rgba(170,192,222,"+(offl?0.14:0.34)+")",1);
  ...
  cavity(g,hcx-13,hby+63,26,9,0.4);
  ...
  var FRONTC=[[-1,15,33,272],[-1,18,40,286],[1,15,33,272],[1,18,0,0]];
  FRONTC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+13, cut=(ci===3);
    var tx,ty,mx,my;
  ...
  (function(){var kc=puh(offl?0.28:0.8);
    ...
    var sx=cx+WS-44, sy=248+SD+breath, ex=sx-8, ey=316+SD+d*0.6, wx=sx+8, wy=382+SD+d*0.5;
    var hy0=362+SD+d*0.4+breath*0.3, kx=cx-(offl?52:54), ky=452+d*0.3,
        ax=cx-40, ay=516+d*0.1, fx=cx-46;
  ...
  (function(){var fc="rgba(150,196,235,"+(offl?0.05:0.14)+")";
    var sx=cx+WS+44, sy=248+SD+breath, ex=sx+8, ey=316+SD+d*0.6, wx=sx-8, wy=382+SD+d*0.5;
    var hy0=362+SD+d*0.4+breath*0.3, kx=cx+(offl?52:46), ky=456+d*0.3,
        ax=cx+24, ay=518+d*0.1, fx=cx+30;
  ...
  return {hand:hh,coreY:275+breath+SD,hy:hby+8+SD,offl:offl,work:work,
          ...
          feet:[{x:cx-46,y:560,w:26,toes:[cx-55,cx-46,cx-37]},
                {x:cx+30,y:560,w:24,toes:[cx+21,cx+30,cx+39]}]};
}

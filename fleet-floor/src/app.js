...
function emptyData(room){return {kind:kindOf(room),queue:[],sessions:[],up:{h:0,m:0},repo:"",spark:[],longest:0,avg:0,success:0,today:0,cur:null,live:true,gh:"unknown",vendor:"unknown",engine:"",cron:{ok:false,last:null,age:null},note:"",paused:false,box:"",logs:[],ping:null,lock:{held:null,stuck:false},expiry:{},authfail:[]};}
...
function liveData(u){
  ...
  d.expiry=u.expiry||{};d.authfail=u.authfail||[];
...
/* Renewal is a row that only exists when there is something to say. A
   permanent "Renew: —" trains the eye to skip the line, and this is precisely
   the line that must be noticed the one month it matters. */
function renewRow(d){
  if(!LIVE||!d.expiry)return "";
  var parts=[],worst=1e9;
  Object.keys(d.expiry).sort().forEach(function(k){
    var e=d.expiry[k];if(!e||typeof e.days!=="number")return;
    if(e.days<worst)worst=e.days;
    parts.push(k+" "+(e.days<0?"EXPIRED":e.days+"d"));
  });
  if(!parts.length)return "";
  return '<span class="k">Renew</span><span class="v" id="v-renew" style="color:'
    +(worst<0?"#ff5147":worst<=7?"#f7bd4e":"#c7d4e4")+'">'+esc(parts.join(" · "))+'</span>';
}

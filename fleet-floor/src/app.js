...
function UNIT(){return AGENT+"-"+ROOM;}
...
function drawMini(t,mx,my,mw,mh){
  ...
  X.fillText(off?"LINK  ———":"QUEUE q"+dataOf(AGENT,ROOM).queue.length,hx+7,hy+42);
...
function drawCell(t,x,y,w,h,unit){var sa=AGENT,sr=ROOM,ss=STATE,sl=lastHand;AGENT=unit.agent;ROOM=unit.room;STATE=unit.state;var info=buildRobo(t,unit.state);lastHand=info.hand;drawMini(t,x,y,w,h);AGENT=sa;ROOM=sr;STATE=ss;lastHand=sl;}
...
function focusUnit(i){var u=ROSTER[i];AGENT=u.agent;ROOM=u.room;STATE=u.state;VIEW="room";syncToggles();refreshChrome();document.body.className="room";populateDash();}
...
function drawHolo(t,st){
  ...
  function panel(c,alpha){c.save();c.globalAlpha=alpha*flick;
    ...
    var dd=dataOf(AGENT,ROOM),up=dd.up.h+"h "+(dd.up.m<10?"0":"")+dd.up.m+"m",ql="q"+dd.queue.length+" · "+dd.repo;
    var rows=offl?[["LINK","— — —","#ff6a62"],["CRON","SILENT","#ff6a62"],["LAST","tick missed",""],["SINCE","00:04:12",""]]
      :st==="working"?[["STATE",ROOM==="builder"?"BUILDING":ROOM==="reviewer"?"REVIEWING":"DISPATCHING","#f7bd4e"],["UPTIME",up,""],["QUEUE",ql,""],["LAST",dd.sessions[0].out,""]]

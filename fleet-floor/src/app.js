...
/* The floor header's counters, as drawn. Split out so FLOORDEV can hand the
   browser walk the same array drawFloorHeader paints — these are canvas text,
   invisible to any DOM assertion, which is how "0 silent" in the collector's
   own log sat next to a page shouting SILENT at three boxes for a week. */
function stageCounts(){
  ...
  /* Prepended so it reads outermost, ahead of the ordinary counts —
     drawFloorHeader paints this array right-aligned from VW-26 and walks
     leftward, so element 0 lands at the right edge — and only when
...
/* Every left-hand header label, as drawn. Kept as data so the browser walk can
   assert canvas text without maintaining a test-only copy of it. */
function floorHeaderLabels(){
  var version=(LIVEMETA&&LIVEMETA.version)||"version unavailable";
  var labels=[
    {text:"FLEET FLOOR",x:26,y:36,font:"700 15px ui-monospace,monospace",color:"#c7d4e4",align:"left"},
    {text:"heavy-duty/crew · fleet call",x:132,y:36,font:"11px ui-monospace,monospace",color:"#5fd6ff",align:"left"},
    {text:version,x:26,y:54,font:"10px ui-monospace,monospace",color:"#7f93aa",align:"left"},
    {text:"scroll · click a unit to zoom in",after:version,x:26,y:54,font:"10px ui-monospace,monospace",color:"#46566a",align:"left"}
  ];
  ...
  X.font="700 11px ui-monospace,monospace";
  var sxp=VW-26;
  stageCounts().forEach(function(s){
    var text="● "+s[0],width=X.measureText(text).width;
    labels.push({text:text,x:sxp,y:36,font:"700 11px ui-monospace,monospace",color:s[1],align:"right",width:width});
    sxp-=width+20;
  });
  X.font="10px ui-monospace,monospace";
  var units=ROSTER.length+" UNITS";
  labels.push({text:units,x:VW-26,y:54,font:"10px ui-monospace,monospace",color:"#46566a",align:"right",width:X.measureText(units).width});
  return labels;
}

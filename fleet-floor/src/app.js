...
function buildGrok(t,st){
  ...
  var REARC=[[-1,5,34,266],[-1,12,46,286],[1,5,34,266],[1,12,46,286]];
  REARC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+8;
    var tx=cx+s*cb[2]+(offl?s*6:0), ty=cb[3]+(offl?30:sway*2.5*(ci%2?1:-1));
    var mx=hcx+s*(cb[1]+24), my=offl?hby+62:hby+40;
    g.strokeStyle="#07090f";g.lineWidth=ci%2?4.8:5.6;g.beginPath();g.moveTo(ax,ay);g.quadraticCurveTo(mx,my,tx,ty);g.stroke();
    ...
    g.fillStyle="#10151d";rr(g,tx-2.6,ty,5.2,7,1);g.fill();
    g.strokeStyle=ed;g.lineWidth=1;rr(g,tx-2.6,ty,5.2,7,1);g.stroke();
    if(ci===1){[RB,RE].forEach(function(c2){c2.fillStyle=pu(offl?0.2:0.75);c2.fillRect(tx-2.6,ty+2.4,5.2,1.6);});}
  });

  // ---------- LEGS: digitigrade — the third segment a walker does not have ----------
  function leg(sgn){
    var hx=cx+sgn*17, hy0=362+d*0.55+breath*0.3;
    var kx=cx+sgn*(offl?42:36), ky=447+d*0.5;
    var ax=cx+sgn*(offl?30:25), ay=514+d*0.2;
    var fx=cx+sgn*32, fy=546;
    // thigh — lean; the mass is tendon, not plate
    limbSeg(g,hx,hy0,kx,ky,10,7,sT,sM);
    tendon(hx+sgn*7,hy0+6,kx+sgn*2,ky-6);
    // shank — thinner still, raked back toward the high ankle
    limbSeg(g,kx,ky,ax,ay,6.5,4.5,sT,sM);
    pl(g,kx-sgn*3,ky+8,ax-sgn*2,ay-4,rc,1);
    // metatarsal — drops forward onto the toes
    limbSeg(g,ax,ay,fx,fy,4.5,6,sM,sB);
    joint(ax,ay,4.5);
    // heel spur behind the ankle
    plate(g,[[ax-sgn*2,ay+3],[ax-sgn*11,ay+9],[ax-sgn*3,ay+14]],sM,sB,null);
    // knee: joint first, pointed guard bolted over it, spike leading
    joint(kx,ky,6);
    plate(g,[[kx-sgn*3,ky-13],[kx+sgn*10,ky-7],[kx+sgn*14,ky+3],[kx+sgn*6,ky+11],[kx-sgn*8,ky+7]],"#2e3a4c",sM,ed);
    rivet(g,kx+sgn*4,ky-1,eH);
    // clawed foot: three toes, PLANTED — the deck contact the old unit never had
    plate(g,[[fx-11,fy],[fx+11,fy],[fx+13,fy+8],[fx-13,fy+8]],sT,sM,ed);
    [-1,0,1].forEach(function(ti){var tx2=fx+ti*8;
      plate(g,[[tx2-3.5,fy+7],[tx2+3.5,fy+7],[tx2+1.6,560],[tx2-1.6,560]],sT,sB,null);
      // bare metal at the claw tip — the paint went first where it meets the deck
      g.fillStyle="rgba(190,208,232,"+(offl?0.18:0.32)+")";g.fillRect(tx2-1.6,557.4,3.2,2.6);});
  }
  leg(-1);leg(1);

  // ---------- PELVIS: narrow — the whole machine tapers to it ----------
  plate(g,[[cx-24,348+d*0.6+breath*0.5],[cx+24,348+d*0.6+breath*0.5],[cx+18,378+d*0.55],[cx-18,378+d*0.55]],sT,sM,ed);
  g.fillStyle="rgba(168,192,226,"+(offl?0.06:0.16)+")";
  poly(g,[[cx-24,348+d*0.6+breath*0.5],[cx+24,348+d*0.6+breath*0.5],[cx+21,353+d*0.6],[cx-21,353+d*0.6]]);g.fill();
  plate(g,[[cx-7,370+d*0.58],[cx+7,370+d*0.58],[cx+5,386+d*0.56],[cx,392+d*0.55],[cx-5,386+d*0.56]],sM,sB,ed);
  rivet(g,cx-4,374+d*0.58,eH);rivet(g,cx+4,374+d*0.58,eH);
  joint(cx-17,366+d*0.55,7);joint(cx+17,366+d*0.55,7);
  ...
  g.fillStyle="#02040a";poly(g,[[cx-15,300+breath],[cx+15,300+breath],[cx+13,348+d*0.6],[cx-13,348+d*0.6]]);g.fill();
  [-1,1].forEach(function(sp){var px=cx+sp*7;
    pl(g,px,303+breath,px,346+d*0.6,"#0a0f18",4);
    pl(g,px-1,303+breath,px-1,346+d*0.6,offl?"#333c48":"#7e94b2",1.2);});
  [-1,1].forEach(function(s){for(var rb2=0;rb2<3;rb2++){var ry=302+rb2*13+breath;
    plate(g,[[cx+s*13,ry],[cx+s*(26-rb2*2.5),ry+3],[cx+s*(25-rb2*2.5),ry+9],[cx+s*12,ry+7]],sT,sM,ed);
    pl(g,cx+s*13,ry+1,cx+s*(25-rb2*2.5),ry+4,"rgba(150,172,204,"+(offl?0.08:0.2)+")",1);}});
  ...
  // ---------- CHEST: compact, angular, forward ----------
  plate(g,[[cx-45,298+breath],[cx-50,250+breath],[cx-22,234+breath],[cx+22,234+breath],[cx+50,250+breath],[cx+45,298+breath]],sT,sM,ed);
  plate(g,[[cx-46,252+breath],[cx-20,238+breath],[cx+20,238+breath],[cx+46,252+breath],[cx+40,261+breath],[cx-40,261+breath]],"#2c3849","#1a2331",null);
  pl(g,cx-34,274+breath,cx-8,274+breath,rc,1);pl(g,cx+8,274+breath,cx+34,274+breath,rc,1);
  rivet(g,cx-40,248+breath,eH);rivet(g,cx+40,248+breath,eH);
  rivet(g,cx-38,292+breath,eH);rivet(g,cx+38,292+breath,eH);
  ...
  [RB,RE].forEach(function(c2){ if(offl){c2.fillStyle="#161b22";c2.fillRect(cx-3,256+breath,6,38);return;}
  ...
  // ---------- COLLAR + fighter neck ----------
  plate(g,[[cx-24,232+breath],[cx+24,232+breath],[cx+17,243+breath],[cx-17,243+breath]],sT,sM,ed);
  plate(g,[[cx-9,218+breath],[cx+9,218+breath],[cx+7,234+breath],[cx-7,234+breath]],sM,sB,null);
  // the neck is two corded tendons under load — short, forward-set, sunk
  [-1,1].forEach(function(s){tendon(hcx+s*5,hby+60,cx+s*7,232+breath,3);});
  ...
  function arm(sgn,mode){
    ...
    var ex,ey;
    if(mode==='reachdown'){ex=sx+sgn*10;ey=sy+40;}
    else if(mode==='raiseup'){ex=sx+sgn*4;ey=sy+42;}
    else {ex=sx+sgn*3;ey=324+d*0.7;}
    tendon(sx+sgn*4,sy+12,ex+sgn*2,ey-4,4);
    tendon(sx-sgn*3,sy+14,ex-sgn*2,ey-4,3);
    ...
    plate(g,P([[-13,-12],[4,-22],[22,-14],[31,2],[20,9],[6,4],[-8,5]]),sT,sM,ed);
    g.fillStyle="rgba(168,192,226,"+(offl?0.07:0.18)+")";
    poly(g,P([[-13,-12],[4,-22],[22,-14],[16,-9],[-10,-7]]));g.fill();
    pl(g,sx+sgn*18,sy-12,sx+sgn*23,sy+1,rc,1.2);
    rivet(g,sx-sgn*5,sy-4,eH);rivet(g,sx+sgn*12,sy-2,eH);
    if(sgn>0){ // a bite out of the right blade's trailing spike — silhouette
      ...
      g.save();g.globalCompositeOperation="destination-out";
      poly(g,[[sx+24,sy+1],[sx+33,sy+4],[sx+26,sy+11]]);g.fill();g.restore();
      pl(g,sx+24,sy+1,sx+26,sy+11,"rgba(170,192,222,"+(offl?0.12:0.32)+")",1);}
    ...
    function claw(x,y,open){
      plate(g,[[x-8,y],[x+8,y],[x+6,y+7],[x-6,y+7]],sM,sB,ed);
      [-1,0,1].forEach(function(f){var fx2=x+f*5.5, len=open?15:11;
        plate(g,[[fx2-2.2,y+6],[fx2+2.2,y+6],[fx2+1.4,y+6+len],[fx2-1.4,y+6+len]],sT,sB,null);
        g.fillStyle="rgba(190,208,232,"+(offl?0.14:0.3)+")";g.fillRect(fx2-1.4,y+4.4+len,2.8,2);});}
    if(mode==='reachdown'){ // builder: reach forward and down onto the work
      var hx3=cx+sgn*56, hy3=330;
      limbSeg(g,ex,ey,hx3,hy3-8,5.5,7,sT,sM);
      ...
      limbSeg(g,ex,ey,fx3,fy3+6,5.5,6.5,sT,sM);
      tendon(ex+sgn*2,ey-2,fx3+sgn*4,fy3+8,3);
      joint(ex,ey-1,5.5);
      claw(fx3,fy3+2,false);
      if(sgn>0)handR={x:fx3+2,y:fy3+4};
    } else { // hanging — long, ending in an open or resting claw near the knees
      var wx=sx-sgn*2, wy=380+d*0.8;
      limbSeg(g,ex,ey,wx,wy,5,7,sT,sM);
      tendon(ex+sgn*3,ey+6,wx+sgn*4,wy-6,3);
      // wrist clamp: the claw is a separate machine, banded on
      plate(g,[[wx-8,wy-2],[wx+8,wy-2],[wx+7,wy+4],[wx-7,wy+4]],sT,sM,ed);
      joint(ex,ey-1,5.5);
      claw(wx,wy+4,offl);
  ...
  var bs=RB.createLinearGradient(0,hby+26,0,hby+36);bs.addColorStop(0,"rgba(0,0,0,0.75)");bs.addColorStop(1,"rgba(0,0,0,0)");
  RB.fillStyle=bs;poly(RB,[[hcx-17,hby+26],[hcx+17,hby+26],[hcx+16,hby+35],[hcx-16,hby+35]]);RB.fill();
  ...
  // was re-seated after
  plate(g,[[hcx-12,hby+44],[hcx+12,hby+44],[hcx+14,hby+58],[hcx+8,hby+65],[hcx-8,hby+65],[hcx-14,hby+58]],sT,sM,ed);
  // the lower lip of the chin catches the lamp — the "rough chin" needs an edge
  pl(g,hcx-7,hby+63.5,hcx+7,hby+63.5,"rgba(170,192,222,"+(offl?0.1:0.28)+")",1.2);
  RB.fillStyle=rc;for(var vs=0;vs<3;vs++)RB.fillRect(hcx-6.5+vs*5,hby+50.5,3.4,7.5);
  g.save();g.globalCompositeOperation="destination-out";
  poly(g,[[hcx-13,hby+56],[hcx-8,hby+58],[hcx-11,hby+62]]);g.fill();g.restore();
  pl(g,hcx-13,hby+56,hcx-11,hby+62,"rgba(170,192,222,"+(offl?0.14:0.34)+")",1);
  ...
  FRONTC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+13, cut=(ci===3);
    ...
    g.strokeStyle="#07090f";g.lineWidth=4.2;g.beginPath();g.moveTo(ax,ay);g.quadraticCurveTo(mx,my,tx,ty);g.stroke();
    ...
    if(cut){ // the stump: no ferrule, just the melt
      g.fillStyle="rgba(220,190,150,"+(offl?0.2:0.55)+")";g.beginPath();g.arc(tx,ty+1,2,0,7);g.fill();
    } else {
      g.fillStyle="#10151d";rr(g,tx-2.4,ty,4.8,6.5,1);g.fill();
      g.strokeStyle=ed;g.lineWidth=1;rr(g,tx-2.4,ty,4.8,6.5,1);g.stroke();
      if(ci===0){[RB,RE].forEach(function(c2){c2.fillStyle=pu(offl?0.2:0.75);c2.fillRect(tx-2.4,ty+2.2,4.8,1.5);});}
    }});

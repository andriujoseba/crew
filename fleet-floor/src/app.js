...
var camStills={}, camFills=0, camCost=0, camSwaps={}, camBuiltAt=[];
var CAM_KEEP=64, CAM_BUDGET=2;
function camPrune(){
  var ks=Object.keys(camStills);if(ks.length<CAM_KEEP)return;
  ks.sort(function(a,b){return camStills[a].at-camStills[b].at;});
  for(var i=0;i<ks.length-CAM_KEEP+1;i++)delete camStills[ks[i]];
}
function camStill(t,u,pw,ph){
  var box=UNITID(u);
  var key=box+"|"+u.agent+"|"+u.room+"|"+u.state+"|"+pw+"x"+ph;
  var e=camStills[key];
  if(e&&e.ok){e.at=mnow();return e;}
  /* CAM_BUDGET survives the cadence's removal as a builds-per-frame throttle
     for miss storms only — resize and scroll-reveal, not steady state. */
  ...
  /* A landed build is a feed swap — drawCamCell performs a glitch over it.
     The dev path (renderMini, camFills forced negative) records none, so a
     golden render of a fixed (unit, state, size, t) stays reproducible. */
  if(camFills>0)camSwaps[box]=t;
...
function drawCamCell(t,x,y,w,h,unit){
  ...
  if(!reduced&&!off&&still&&still.ok){
    ...
    if(gk===0){ // displaced slices, the roll's own move at feed strength
      ...
      X.drawImage(still.cv,x+1-3*gs,y+1,w-2,h-2);
      X.drawImage(still.cv,x+1+3*gs,y+1,w-2,h-2);
      X.restore();
...
window.FLOORDEV={W:DW,H:DH,AGENTS:["claude","codex","grok","kimi"],
  ...
  renderMini:function(dst,o){
    var sX=X,sdpr=dpr,sfl=camFills;
    var k=dst.width/CAMREF.W;
    X=dst.getContext("2d");dpr=k;
    /* No budget on a map: every tile needs its portrait THIS call, not when
       the app's per-frame ration next comes round. */
    camFills=-1e9;

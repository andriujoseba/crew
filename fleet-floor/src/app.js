...
function buildGrok(t,st){
  ...
  var REARC=[[-1,13,36,268],[-1,18,48,286],[-1,23,60,300],[1,13,36,268],[1,18,48,286],[1,23,60,300]];
  stanceOn();
  REARC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+10, rank=ci%3;
    var tx=cx+s*cb[2]+(offl?s*6:0), ty=cb[3]+(offl?30:sway*2.5*(ci%2?1:-1));
    var mx=hcx+s*(cb[1]+(rank===2?34:26)), my=offl?hby+62:hby+38;
    g.strokeStyle="#07090f";g.lineWidth=5.8-rank*0.7;g.beginPath();g.moveTo(ax,ay);g.quadraticCurveTo(mx,my,tx,ty);g.stroke();
    // the key is upper-LEFT: left cables carry the light, right cables sit dark
    g.strokeStyle=offl?"#20242c":(s<0?"#4e5a6e":"#242b36");g.lineWidth=s<0?1.5:1.1;g.beginPath();g.moveTo(ax,ay-1);g.quadraticCurveTo(mx,my-2,tx-s,ty-1);g.stroke();
    // ferrule: the machined weight that keeps a cable hanging straight
    var fw3=6-rank*0.5;
    ...
    if(rank===1){ // ring band mid-length — the reference dreads carry metal rings
      var qt=0.52, qx=(1-qt)*(1-qt)*ax+2*qt*(1-qt)*mx+qt*qt*tx, qy=(1-qt)*(1-qt)*ay+2*qt*(1-qt)*my+qt*qt*ty;
      g.fillStyle="#232b38";g.fillRect(qx-3,qy-1.6,6,3.2);
      pl(g,qx-3,qy-1.4,qx+3,qy-1.4,"rgba(150,172,204,"+(offl?0.1:0.3)+")",0.9);}
  });
  stanceOff();
  ...
  function arm(sgn,mode){
    ...
    if(mode==='reachdown'){ /* builder: the FLEX — elbow thrown out past the
  ...
  /* The rooms park a readout on the right half of the bench, exactly where a
     ...
     handheld rooms. */
  arm(-1, (work&&ROOM==="builder")?'reachdown':'down');
  arm(1, (work&&ROOM!=="builder")?'raiseup':'down');
  if(work&&ROOM!=="builder"&&handR){var h=handR;
    if(ROOM==="reviewer"){ plate(g,[[h.x-3,h.y-9],[h.x+17,h.y-11],[h.x+17,h.y+2],[h.x-3,h.y+4]],"#1a2836","#0c1620","#3a5570");
  ...
  FRONTC.forEach(function(cb,ci){var s=cb[0], ax=hcx+s*cb[1], ay=hby+16, cut=(ci===3);
    var tx,ty,mx,my;
    if(cut){tx=hcx+s*26;ty=hby+44;mx=hcx+s*22;my=hby+26;}
    else if(offl){tx=hcx+s*(6+(ci%2)*5);ty=hby+62+(ci%2)*6;mx=hcx+s*17;my=hby+34;}
    else {tx=cx+s*cb[2];ty=cb[3]+sway*2*(ci%2?-1:1);mx=hcx+s*(cb[1]+15);my=hby+38;}
    g.strokeStyle="#07090f";g.lineWidth=4.8;g.beginPath();g.moveTo(ax,ay);g.quadraticCurveTo(mx,my,tx,ty);g.stroke();
    // same one-key rule as the rear rank: lit highlight left, dark right
    g.strokeStyle=offl?"#20242c":(s<0?"#4e5a6e":"#242b36");g.lineWidth=s<0?1.4:1.0;g.beginPath();g.moveTo(ax-s,ay);g.quadraticCurveTo(mx-s,my-1,tx-s,ty-1);g.stroke();

...
function populateDash(){
  ...
  document.getElementById("w-queue").innerHTML='<div class="wt"><span class="dot"></span>WORK QUEUE · q'+d.queue.length+'</div><div class="qchips">'
    ...
    /* #486 D4 — a DISTINCT control that names its own violence, on its own
       ...
       button on the panel. */
    +'<button class="lbtn'+(LIVE?'':' woff')+'" id="ac-force"'+(LIVE?'':' title="'+CTL_TIP+'"')+' style="text-align:center;color:#ff5147;border-color:#3a1c1c">&#10006; &nbsp;Force stop &middot; kills the guest</button></div>';
...
/* unreachableBox DATA — has this box stopped answering, for the WORDS on a
   ...
   wedged() is the rule, and restart asks it there. This is the page's read of
   the same published facts, so the confirm an operator gets describes the path
   the host will take rather than a friendlier one.

   `stale` is excluded here and not in alertOf above, deliberately: a tier that
   has not run recently enough is unmeasured, and telling an operator their box
   is about to be killed on an unmeasured hunch is the one direction this
   sentence must not be wrong in. */
function unreachableBox(d){return !!(LIVE&&d&&d.ping&&!d.ping.ok&&!d.ping.stale&&d.ping.fails>=PING_FAILS_SHOWN);}

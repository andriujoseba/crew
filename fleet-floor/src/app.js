...
function buildGrok(t,st){
  ...
  function arm(sgn,mode){
    ...
    function claw(x,y,open){
      plate(g,[[x-9.5,y],[x+9.5,y],[x+7.5,y+8],[x-7.5,y+8]],sM,sB,ed);
      [-1,0,1].forEach(function(f){var fx2=x+f*6.2, len=open?17:13;
        plate(g,[[fx2-2.8,y+7],[fx2+2.8,y+7],[fx2+1.8,y+7+len],[fx2-1.8,y+7+len]],sT,sB,null);
        g.fillStyle="rgba(190,208,232,"+(offl?0.14:0.3)+")";g.fillRect(fx2-1.8,y+5.4+len,3.4,2);});}
    if(mode==='reachdown'){ /* builder: the FLEX — elbow thrown out past the
      ...
      if(sgn>0)handR={x:fx3+2,y:fy3+4};
    } else { // hanging — the forearm is a CONE: narrow off the elbow, all the
      // mass arriving at the wrist, where a machine that grips carries it
      var wx=sx-sgn*2, wy=380+d*0.8;
      limbSeg(g,ex,ey,wx,wy,6,9.5,sT,sM);
      // bracer fin off the outer flare
      plate(g,[[wx+sgn*8,wy-26],[wx+sgn*15,wy-14],[wx+sgn*9,wy-6]],sM,sB,ed);
      tendon(ex+sgn*3,ey+6,wx+sgn*5,wy-8,3);
      pl(g,ex-sgn*4,ey+8,wx-sgn*7,wy-6,rc,1);
      // wrist clamp: the claw is a separate machine, banded on
      plate(g,[[wx-10,wy-2],[wx+10,wy-2],[wx+9,wy+4],[wx-9,wy+4]],sT,sM,ed);
      ...
      if(sgn<0){g.save();g.globalAlpha=offl?0.25:0.5;g.fillStyle="#c9a227";
        for(var tl2=0;tl2<4;tl2++)g.fillRect(wx-4+tl2*3,wy-26,1.6,7);
        g.restore();}
      if(sgn>0)handR={x:wx,y:wy+14};
    }
  }
  ...
  return {hand:hh,coreY:275+breath,hy:hby+8,offl:offl,work:work,
          feet:[{x:cx-30,y:560,w:24},{x:cx+30,y:560,w:24}]};
}

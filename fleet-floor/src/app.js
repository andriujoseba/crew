...
/* ===================== CONFERENCE CELL (god-view webcam tile) =====================
   ...
   different lifetimes:
     ...
     portrait  — buildRobo cropped to head-and-shoulders, cached per unit and
                 refreshed on a budget. The refresh IS the webcam frame rate:
                 a feed that updates a few times a second reads as live video,
                 so the budget is a feature here, not a compromise.
     overlays  — AR-style telemetry drawn crisp every frame: state, uptime,
                 ...
                 wait on the portrait's cadence. */
...
/* ---- the portrait ----------------------------------------------------------
   ...
   still costs a sprite build instead of a full room render — cheap enough
   that every visible feed refreshes a few times a second. Framing hangs off
   the one face anchor every vendor already reports (`hy`, the visor line),
   with a per-vendor crop width because a spider and a humanoid do not wear
   their shoulders at the same span. */
...
var CAMFRAME={claude:{w:250,dy:0,fy:0.40},codex:{w:260,dy:14,fy:0.46},
              grok:{w:206,dy:-2,fy:0.36},kimi:{w:330,dy:-38,fy:0.42}};
function mnow(){return (window.performance&&performance.now)?performance.now():0;}
var camStills={}, camFills=0, camCost=0;
var CAM_KEEP=64, CAM_BUDGET=2, CAM_SLOW_MS=14, CAM_TTL_SLOW=3.5;
var CAM_TTL={working:0.30,idle:0.6,offline:2.5};
function camPrune(){
...
function camStill(t,u,pw,ph){
  ...
  var e=camStills[key];
  var ttl=camCost>CAM_SLOW_MS?CAM_TTL_SLOW:(CAM_TTL[u.state]||0.9);
  if(e&&(t-e.at)<ttl*e.jit)return e;
  if(camFills>=CAM_BUDGET)return e||null;
  camFills++;
  if(!e){camPrune();e=camStills[key]={cv:oc(pw,ph),at:0,jit:0.75+0.5*(strPhase(key)/6.283)};}
  var t0=mnow(), sa=AGENT, ss=STATE;
  AGENT=u.agent;STATE=u.state;
  var info=buildRobo(t,u.state);
  ...
  /* The camera is MOUNTED. hy rides the sprite's own idle motion — grok
     reports half its breath, kimi its whole hover bob — so a crop recomputed
     per refresh TRACKS the chest and cancels on screen exactly the life it
     exists to show: kimi hovered perfectly still, claude sat frozen, grok
     breathed at half height. Lock the mount on a key's first frame; from
     then on the unit moves inside a fixed frame, like anything on a tripod. */
  if(e.sy0===undefined)e.sy0=info.hy+f.dy-sh*f.fy+jy;
  var sy=e.sy0;
  var c=e.cv.getContext("2d");
  /* Codex's bob is Math.round()ed for the room — at portrait zoom that is a
     1px stutter while everyone else sweeps. Top it up with the smooth curve
     the round() threw away, applied to the whole draw so body and glow move
     together. Offline units get nothing: dead is still. */
  var dy3=(u.agent==="codex"&&!off)?Math.sin(t*1.5)*ph*0.008:0;
  ...
  c.drawImage(robo,sx,sy,sw,sh,0,dy3,pw,ph);
  c.filter="none";
  c.globalCompositeOperation="lighter";
  c.globalAlpha=off?0.4:0.9;c.drawImage(rrim,sx,sy,sw,sh,0,dy3,pw,ph);
  ...
  c.globalAlpha=0.55;c.filter="blur("+bl+"px)";c.drawImage(remit,sx,sy,sw,sh,0,dy3,pw,ph);
  c.globalAlpha=0.45;c.filter="blur("+bl*3+"px)";c.drawImage(remit,sx,sy,sw,sh,0,dy3,pw,ph);
  c.globalAlpha=off?0.5:0.9;c.filter="none";c.drawImage(remit,sx,sy,sw,sh,0,dy3,pw,ph);
...
function drawCamCell(t,x,y,w,h,unit){
  ...
  if(still&&still.at)X.drawImage(still.cv,x+1,y+1,w-2,h-2);
  /* Live sensor grain — the one per-frame texture, and what says "feed"
     rather than "poster" between portrait refreshes. */
...
window.FLOORDEV={W:DW,H:DH,AGENTS:["claude","codex","grok","kimi"],
  ...
  /* The floor camera (vertical, in px of grid overflow), for the browser
     walk. The camera eases per FRAME, not per second, and a console dwell
     expires every visible portrait still — so the first floor frames after
     Escape re-render sprites, and a fixed post-wheel wait covers almost no
     easing on a slow machine. The walk polls this instead of sleeping; it
     asserts by outcome either way. */

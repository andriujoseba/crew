...
function drawMini(t,mx,my,mw,mh){
  ...
  /* The cell floor. This is the view an operator actually scans — the god-view
     is the default — and it was a flat #04070d rectangle with a black bar,
     which is exactly what the full room's floor was before it got one. The
     room treatments are the same three ideas at cell scale: stained concrete,
     a reflective sealed floor, a painted dispatch lane. */
  var fH=dy0+dH-fY, mwarm=ROOM==="builder", mcool=ROOM==="reviewer";
  var mb=mwarm?[46,38,28]:mcool?[31,42,56]:[37,40,47], mk=off?0.45:1;
  var mfg=X.createLinearGradient(0,fY,0,dy0+dH);
  mfg.addColorStop(0,rgba(mb[0]*mk,mb[1]*mk,mb[2]*mk,1));
  mfg.addColorStop(1,"#03060c");
  X.fillStyle=mfg;X.fillRect(dx0,fY,dW,fH);
  X.fillStyle="rgba(0,0,0,0.5)";X.fillRect(dx0,fY-2,dW,2);
  X.fillStyle=rgba(150,180,215,0.13*mk);X.fillRect(dx0,fY,dW,1);
  if(mwarm){[[0.28,0.30],[0.62,0.22],[0.86,0.26]].forEach(function(o){
      var ox=dx0+dW*o[0],oy=fY+fH*0.34,orad=dW*0.10;
      var og=X.createRadialGradient(ox,oy,1,ox,oy,orad);
      og.addColorStop(0,"rgba(6,5,4,"+(o[1]*mk)+")");og.addColorStop(1,"rgba(6,5,4,0)");
      X.fillStyle=og;X.beginPath();X.ellipse(ox,oy,orad,orad*0.34,0,0,7);X.fill();});
  } else if(mcool){X.save();X.globalCompositeOperation="lighter";
      var mrf=X.createLinearGradient(0,fY,0,fY+fH*0.7);
      mrf.addColorStop(0,"rgba(120,180,240,"+(0.09*mk)+")");mrf.addColorStop(1,"rgba(120,180,240,0)");
      X.fillStyle=mrf;X.fillRect(dx0,fY,dW,fH*0.7);X.restore();
  } else {X.save();X.globalAlpha=off?0.25:0.55;X.strokeStyle="rgba(201,162,39,0.34)";X.lineWidth=1;
      X.beginPath();X.moveTo(dx0,fY+fH*0.22);X.lineTo(dx0+dW,fY+fH*0.22);
      X.moveTo(dx0,fY+fH*0.68);X.lineTo(dx0+dW,fY+fH*0.68);X.stroke();
      for(var mcx=dx0+8;mcx<dx0+dW;mcx+=26){X.lineWidth=1.6;X.beginPath();
        X.moveTo(mcx,fY+fH*0.34);X.lineTo(mcx+5,fY+fH*0.45);X.lineTo(mcx,fY+fH*0.56);X.stroke();}
      X.restore();}
  ...
  /* Contact shadows in the cell. The grid drew every unit with NONE — all four
     floated, in the one view an operator actually scans. Same footprints and
     the same height rule the full room uses, read off lastFeet, so claude and
     codex plant and grok and kimi hover here too rather than all four being
     equally weightless. */
  var mfeet=lastFeet||[{x:260,y:560,w:90}];
  var mfloat=(AGENT==="grok"||AGENT==="kimi");
  X.save();
  mfeet.forEach(function(f){
    var mfx=rpx+f.x*mrs, mfy=rpy+f.y*mrs, mfw=Math.max(2.4,f.w*mrs);
    var mlift=Math.max(0,fY-mfy), msp=1+Math.min(2.6,mlift/13);
    var ma=(mfloat?0.42:0.62)/Math.pow(msp,1.25)*(off?0.5:1);
    var mw2=mfw*msp;
    var msh=X.createRadialGradient(mfx,fY+2,0.5,mfx,fY+2,mw2);
    msh.addColorStop(0,"rgba(0,0,0,"+ma.toFixed(3)+")");
    msh.addColorStop(0.55,"rgba(0,0,0,"+(ma*0.45).toFixed(3)+")");
    msh.addColorStop(1,"rgba(0,0,0,0)");
    X.fillStyle=msh;X.beginPath();X.ellipse(mfx,fY+2,mw2,Math.max(1.2,mfw*0.34*msp),0,0,7);X.fill();
    if(mlift<3&&!mfloat){X.fillStyle="rgba(0,0,0,0.5)";X.beginPath();X.ellipse(mfx,fY+1,mfw*0.62,Math.max(0.9,mfw*0.2),0,0,7);X.fill();}
  });
  X.restore();
  // ROBOT hero
  ...
  /* The offline panel's flicker was raw Math.random(). Invisible in the app —
     and fatal to any MAP of the app: all twelve offline tiles would differ
     between two renders of the same commit, which is the exact trap the film
     grain sprang on the room map before dev/seed.js existed. Same irregular
     blink, driven by time and the unit's own name. */
  var flp=strPhase(BOX),flw=Math.sin(t*7.7+flp)*0.6+Math.sin(t*13.1+flp*1.7)*0.4;
  var hx=dx0+dW*0.60,hy=dy0+dH*0.28,hw=dW*0.33,hh=dH*0.40,C0=off?[255,74,66]:[95,214,255],fl=off?(flw>0.62?0.4:0.9):0.95,cs=C0[0]+","+C0[1]+","+C0[2];
...
function drawCell(t,x,y,w,h,unit){var sa=AGENT,sr=ROOM,ss=STATE,sl=lastHand,sb=BOX,sf=lastFeet;AGENT=unit.agent;ROOM=unit.room;STATE=unit.state;BOX=UNITID(unit);var info=buildRobo(t,unit.state);lastHand=info.hand;lastFeet=info.feet;drawMini(t,x,y,w,h);AGENT=sa;ROOM=sr;STATE=ss;lastHand=sl;lastFeet=sf;BOX=sb;}
...
/* ---- dev hook: the room renderer, addressable one unit at a time ----------
   ...
   same drawTarget the room view uses, into a canvas the caller owns, and puts
   back every global it borrowed: VIEW, the roster and the live poll never see
   it. Unused by index.html — it costs one property on window. */
window.FLOORDEV={W:DW,H:DH,AGENTS:["claude","codex","grok","kimi"],
  ...
  render:function(dst,o){
    var sa=AGENT,sr=ROOM,ss=STATE,sb=BOX,sl=lastHand;
    AGENT=o.agent;ROOM=o.room;STATE=o.state;BOX=o.agent+"-"+o.room;
    /* The lamp flicker and the weld sparks are the only two things that carry
       state across frames, so both are reset and then warmed a FIXED number of
       frames — otherwise a tile inherits however many frames the previous
       caller happened to run, and the grid stops being comparable. */
    ...
    AGENT=sa;ROOM=sr;STATE=ss;BOX=sb;lastHand=sl;
  }};
})();

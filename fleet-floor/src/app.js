...
function buildGrok(t,st){
  ...
  (function(){var fc="rgba(150,196,235,"+(offl?0.14:0.36)+")";
    var sx=cx+44, sy=248+breath, ex=sx+8, ey=316+d*0.6, wx=sx-8, wy=395+d*0.5;
    var hy0=362+d*0.55+breath*0.3, kx=cx+(offl?44:38), ky=447+d*0.5,
        ax=cx+(offl?31:26), ay=514+d*0.2, fx=cx+34;
    ...
    try{
      ...
      (info.feet||[]).forEach(function(f2){
        ...
        (f2.toes||[]).forEach(function(tx5){
          var txd=mx2(tx5), tr=Math.max(3,sc2*5), tcy=fyd+2.5;
          var tg=c.createRadialGradient(txd,tcy,0.5,txd,tcy,tr);
          tg.addColorStop(0,"rgba(0,0,0,0.85)");tg.addColorStop(0.55,"rgba(0,0,0,0.45)");tg.addColorStop(1,"rgba(0,0,0,0)");
          c.save();c.translate(txd,tcy);c.scale(1,0.38);c.translate(-txd,-tcy);
      ...
      var bl2=Math.max(2,Math.round(W2/90));
      c.globalAlpha=0.55;c.filter="blur("+bl2+"px)";c.drawImage(remit,sxp,syp,swp,shp,dxp,dyp,dwp,dhp);
      c.globalAlpha=0.45;c.filter="blur("+bl2*3+"px)";c.drawImage(remit,sxp,syp,swp,shp,dxp,dyp,dwp,dhp);

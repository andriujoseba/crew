...
`install.sh` from the box's `fleet.roster` row (or from `--role/--agent`
...
composed per box by its single `fleet.roster` row (or by explicit
`--role/--agent` flags for an off-roster bake), resolved into
...
2. The operator runs the logins interactively (creds-free-by-default is a
   ...
   credentials itself).
3. `crew hire` resolves the box's profiles from its roster row and it goes
   on duty; authentication authorizes GitHub, it does not select a role.
...
- *Builders scale trivially.* One box per identity stays the invariant, so
  "another claude builder" = a new GitHub identity + one roster line. The

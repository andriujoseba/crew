...
session_reconcile_orphans() {
  ...
  while IFS=$'\037' read -r kind key holder slog started; do
    ...
    log "SESSION END kind=$kind key=$key rc=- dur=- outcome=$SESSION_ORPHAN_OUTCOME acted=unknown reply_tail= tier=unknown peak_rss=- input_tokens=- output_tokens=- cache_creation_input_tokens=- cache_read_input_tokens=- cost_usd=- session_id=unknown pool=unknown started=$started" >>"$logfile"

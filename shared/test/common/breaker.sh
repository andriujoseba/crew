...
# shellcheck disable=SC2016,SC2030,SC2031,SC2034,SC2317
budget_corrupt_fails_closed() ( # budget_corrupt_fails_closed header|entry|latch
...
# shellcheck disable=SC2016,SC2030,SC2031,SC2034,SC2317
budget_report_says_unreadable() (
  local bdir="$TMP/budget-report-bad"
  mkdir -p "$bdir/logs"
  DUTY_DIR="$bdir"
  printf 'this is not a budget header\n' >"$bdir/.session-budget.review"
  # The one state that must not render as a balance: the gate is refusing
  # every dispatch on this lane, and the report is what connects a silent box
  # to its cause.
